...
<Header name={userInfo.displayName} photoURL={userInfo.photoURL} signout={signout}  />
  <PostSender/>
  <Typography variant="h3" style={{textAlign:"center"}}>Issues</Typography>
  {
  post.map((info,index) =>  
    <Post key={info.id} showButton={true} title={info.data.postTitle} body={info.data.postBody} name={userInfo.displayName} photoURL={userInfo.photoURL} index={index} id={info.id} />
  )
}
...
<Header name={userInfo.displayName} photoURL={userInfo.photoURL} signout={signout}  />
    ...
    <PostDetail name={userInfo.displayName}  photoURL={userInfo.photoURL}/>
  </PrivateRoute>
</Switch>
